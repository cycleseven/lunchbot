...
def test_should_return_working_month():
    ...
    event = LunchbotMessageEvent({"ts": get_timestamp(2018, 11, 1)})
    ...
    event = LunchbotMessageEvent({"ts": get_timestamp(2018, 10, 30)})
    assert "11/2018" == event.get_working_month()
    ...
    event = LunchbotMessageEvent({"ts": get_timestamp(2018, 12, 31)})
